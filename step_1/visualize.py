"""
Bug Simulation Visualizer (PPO Version)
Usage: python visualize_ppo.py [path_to_checkpoint.pt]
       python visualize_ppo.py  (will prompt for path)
...
# ── Visualizer Config ──────────────────────────────────────────────────────────
# Set these to match what you used in train.py so the UI looks right
class VizConfig:
    grid_size     = 24
    min_food      = 100   # Set lower if you want to watch them hunt in a sparse map!
    food_zones    = [{"x": 8, "y": 8, "w": 16, "h": 16}]
    max_life      = 400.0
    food_reward   = 40.0  # Internal life boost amount 
    life_decay    = 1.0
    sensor_radius = 5
    fov_degrees   = 120
    front_fov_radius = 5
...
class SimState:
    ...
    def _load(self):
        ...
        # 1. Spin up the World (with num_envs=1)
        self.world = World(
            num_envs         = 1,
            grid_size        = VizConfig.grid_size,
            food_zones       = VizConfig.food_zones,
            min_food         = VizConfig.min_food,
            sensor_radius    = VizConfig.sensor_radius,
            fov_degrees      = VizConfig.fov_degrees,
            front_fov_radius = VizConfig.front_fov_radius,
            side_fov_radius  = VizConfig.side_fov_radius,
            max_life         = VizConfig.max_life,
            food_reward      = VizConfig.food_reward,
            life_decay       = VizConfig.life_decay,
            device           = self.device,
        )

        # 2. Spin up the Brain & load weights
        self.brain = BugActorCritic(
            num_sensors = self.world.num_sensors, 
            hidden_dim  = VizConfig.hidden_dim
        ).to(self.device)
        
        checkpoint = torch.load(self.checkpoint_path, map_location=self.device)
        self.brain.load_state_dict(checkpoint["model"])
        self.brain.eval() # Set to evaluation mode

        # 3. Initial State
        self.memory = self.brain.init_memory(num_envs=1, device=self.device)
        ...
        print(f"[Viz] Loaded Update {checkpoint.get('update', '?')} | Total Steps {checkpoint.get('total_steps', '?'):,}")
        print(f"[Viz] World {VizConfig.grid_size}×{VizConfig.grid_size}, max_life={VizConfig.max_life}")
    ...
    def step(self):
        ...
        # 1. Brain thinks (Batched for 1 env)
        with torch.no_grad():
            action, _, _, self.memory = self.brain(self.obs, self.memory)
            
        self.last_action = action[0].item()
        
        # 2. World steps
        self.obs, reward, done = self.world.step(action)
        
        new_life = self.world.life_force[0].item()
        
        # 3. Track food eaten. If life decays by 1, any delta > -1 means they ate!
        delta_life = new_life - self.last_life
        if delta_life > -self.world.LIFE_DECAY + 0.1:
            self.food_eaten += 1
    ...
    # Grab the vision slice of the batched obs tensor
    @property
    def vision_1d(self): return self.obs[0, :-1].cpu()                   
...
def draw_world(surf, sim, cell, ox, oy, cone_surf):
    ...
    # Draw grid cells (WALL logic removed as it's not in the new world)
    for gy in range(gs):
        for gx in range(gs):
            v = grid[gy, gx].item()
            if v == World.FOOD:
...
def draw_panel(surf, sim, fonts, btns, slider, panel_x):
    ...
    text(surf, "BUG  VIEWER (PPO)", f_lg, C_TEXT_HEAD, px, y)
    y += f_lg.get_height() + 4
    text(surf, f"RL Agent · {sim.grid_size}×{sim.grid_size} world", f_xs, C_TEXT_DIM, px, y)
...
def main(checkpoint_path):
    ...
    pygame.init()
    pygame.display.set_caption("Bug Visualizer PPO")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bug Simulation Visualizer (PPO)")
    parser.add_argument("checkpoint", nargs="?", default="",
                        help="Path to .pt checkpoint file (e.g. checkpoints/bug_final.pt)")
